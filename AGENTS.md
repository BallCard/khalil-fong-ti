...
**手机端兼容优化**：
- 增大触摸区域：按钮最小高度提升至 48px，选项卡片增加内边距
- 增强触摸反馈：添加 `-webkit-tap-highlight-color` 优化触摸高亮，添加 `active` 状态缩放效果
- 改善响应式布局：针对 600px 以下屏幕优化字体、内边距、间距
- 添加极小屏幕适配（380px 以下）
- 移除 hover 效果在触摸设备上的误触发，改为纯 active 状态

**分享截图界面优化**：
- 预览面板宽度在移动端调整为 95vw
- 按钮布局在移动端改为垂直堆叠
- 根据设备类型显示不同提示文字（移动端更简洁）
- "打开原图"按钮文字改为"全屏查看并保存"
- 全屏图片查看模式的提示文字优化
